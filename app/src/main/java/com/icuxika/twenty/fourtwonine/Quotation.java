...
public record Quotation(String agency, int amount) implements PageComponent {
    ...
    public static ScopedValue<String> LICENSE = ScopedValue.newInstance();

    public Quotation {
        if (!(LICENSE.isBound() && LICENSE.get().equals("License A"))) {
            throw new IllegalStateException("No license found");
        }
    }
